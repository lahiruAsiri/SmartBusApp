...
export const DriverFinesScreen = ({ navigation }: any) => {
  ...
  const fetchNotes = async () => {
    ...
    try {
      const q = query(
        ...
        where('driverName', '==', userData.displayName)
      );
      ...
      console.error("Error fetching driver fines:", error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
      ...
      <View style={[styles.card, { backgroundColor: colors.card }]}>
        <View style={styles.cardHeader}>
          <View style={styles.busInfo}>
            ...
            <Text style={[styles.violationTitle, { color: colors.text }]}>{item.violationType}</Text>
          </View>
          <View style={[
            styles.statusBadge, 
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={[styles.infoText, { color: colors.text }]}>Bus: {item.busNumber}</Text>
        </View>
        
        <View style={styles.infoRow}>
          <Ionicons name="person-circle" size={16} color="#3B82F6" />
          <Text style={[styles.infoText, { color: colors.text }]}>Officer: {item.officerName || 'Police'}</Text>
        </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { backgroundColor: colors.primary }]}>
        ...
        <Text style={styles.headerTitle}>My Police Fines</Text>
        <View style={{ width: 26 }} />
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.centerContainer}>
          ...
          <Text style={[styles.emptyText, { color: colors.text }]}>No fines on your record.</Text>
          <Text style={styles.emptySubText}>Keep up the good driving!</Text>
        </View>
      ) : (
        <FlatList
          data={notes}
...
const styles = StyleSheet.create({
  ...
  violationTitle: { fontSize: 18, fontWeight: 'bold' },
